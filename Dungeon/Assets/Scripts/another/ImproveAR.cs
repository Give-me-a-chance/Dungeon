...
public class ImproveAR : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        lvlDamageNow = int.Parse(Stats.LvlsAR.Split(' ')[0]);
        lvlFireRateNow = int.Parse(Stats.LvlsAR.Split(' ')[1]);
        ...
        */
        ...
        if (lvlDamageNow > 1)
            DamageCount.text = (30 + lvlDamageNow * damagechange).ToString();
        else
            DamageCount.text = "30";
        if (lvlDamageNow < 5)
        {
            DamageLvl[lvlDamageNow].color = Color.white;
            damageprice.text = (2000 * (lvlDamageNow + 1)).ToString();
        ...
        if (lvlFireRateNow < 5)
        {
            ...
            FrPrice.text = (500 * (lvlFireRateNow + 1)).ToString();
    ...
    public void ImproveDamage_Click()
    {
        if (Stats.GoldCount >= int.Parse(damageprice.text))
        {
            ...
            Stats.LvlsAR = $"{(int.Parse(Stats.LvlsAR.Split(' ')[0]) + 1).ToString()} {lvlFireRateNow}";
            Debug.Log(Stats.LvlsAR);
            con.UpAR();
        }
        Start();
    }
    ...
    public void ImproveFR_Click()
    {
        if (Stats.GoldCount >= int.Parse(FrPrice.text))
        {
            ...
            Stats.LvlsAR = $"{lvlDamageNow} {(int.Parse(Stats.LvlsAR.Split(' ')[1]) + 1).ToString()}";
            Debug.Log(Stats.LvlsAR);
            con.UpAR();
        }
        Start();
